...
module.exports =  (payload, conn) => {
    let {vendor_id, order_state, limit} = payload;
        ...
        try {
            let results = 
                await conn.query(`SELECT p.id, p.uuid as product_id, o.order_state as status, c.uuid as catalog_id, o.created_at, 
                        ...
                        INNER JOIN tb_productdetail pd ON p.id=pd.product_id AND pd.lang_id=3
                        ...
                        INNER JOIN tb_vendorchannel as vc ON o.channel_id=vc.id
                        WHERE vc.vendor_id=? AND o.order_state in ${order_state} 
                        ORDER BY o.created_at DESC LIMIT ?`, [vendor_id, limit]);
            ...
            let p_result = results[0].map(async(item)=>{
                let productDetail = 
                await conn.query(`SELECT pd.detail_shortdesc, l.lang_code as lang FROM tb_productdetail pd 
                                        INNER JOIN tb_lang l ON pd.lang_id=l.id
                                        WHERE pd.product_id=?`, [item.id]);
                return ({
                    name: item.name,
            ...
            logger.log({level: "warn", message: `LastSoldItems Error. Error: ${err.message}`})
            return reject(err);
        }
    })
}